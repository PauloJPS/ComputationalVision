...
class cortex:
    def __init__(self, n, m, a, r):
        self.__cortexS= n #cortex size
    ...
    def initializeWeights(self):
        self.Ws = np.random.rand(self.__cortexS, self.__cortexS, self.__retinS)
    ...
    def updateWeights(self):
        ...
        delta = self[iW, jW] - self.im
        self.Ws[iW, jW] = self.Ws[iW, jW] - self.__a * delta
        for i in self.__circle:
            r = (i[0]**2 + i[1]**2)**0.5
            self.Ws[i[0] + iW, i[1] + jW] = self.Ws[i[0] + iW, i[1] + jW] - self.__a * np.exp(-self.__r * r) * delta
